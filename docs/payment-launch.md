...
| Relay | 10 | Network fees for payment submission and receipt anchoring. Separate limited-balance operational signer. Secure provisioning is still required; a public address alone cannot sign. |
...
On 24 September 2026, the seven contract creations totalled 6,421,105 estimated gas at 20 Gwei: **0.1284221 USDC**. Adding an assumed 2,000,000 gas for configuration and a fivefold fee buffer gives **0.8421105 USDC**. The administrator's 2 USDC reserve rounds this up with additional headroom. Configuration gas is an allowance, not a simulation of a finished deployment. Supply, allocation and policy constructor arguments remain subject to approval.
...
- Produce a mainnet deployment plan with explicit owners and addresses. Existing `contracts:deploy` is a local-only workflow using mock contracts and bootstrap approvals; it must not be repurposed by changing its RPC. Current UsageMeter settlement/recovery authority is its deployment owner. Separate governance and relay roles must be addressed explicitly before deployment; this checklist does not implement that separation.
...
- Complete wallet ownership/authentication, signed payment checkout, real-token settlement and recovery acceptance. The receipt signer remains in software until confidential infrastructure is available; keep that limitation visible.
...
- Keep treasury-to-relay replenishment manual initially. Automatic replenishment, balance alerts and gas-spend caps are not currently configured. They require explicit limits and an agreed funding mechanism; never give the service wallet unrestricted treasury access.

Deployment and paid tests wait for addresses, secure signing access, funding, approved parameters and a working inference path. The operator need not configure WalletConnect again.
